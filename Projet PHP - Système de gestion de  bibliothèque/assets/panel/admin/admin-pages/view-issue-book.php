...
if (isset($_POST['book-return'])) {
  ...
  $isbn = $fetch['isbn'];
  if ($query) {
    $error['book-msg'] = "Le livre a été retourné avec succès";
...
?>
  ...
  <?php
    ...
    $error['book-msg'] = "Une erreur est survenue lors du retour du livre";
  ?>
    <script>
      setTimeout(() => {
        location.replace("view-issue-book.php");
...
if (isset($_POST['send-mail'])) {
  $email = $_POST['email'];
  $msg = $_POST['msg'];
  $subject = "Rappel pour le retour du livre";
  $sender = "De: codewithpawanofficial@gmail.com";
  if (mail($email, $subject, $msg, $sender)) {
    $error['book-msg'] = "L'e-mail a été envoyé avec succès à " . $email;
    ?>
    <script>
      setTimeout(() => {
        location.replace("view-issue-book.php");
  ...
  <?php
    ...
    $error['book-msg'] = "Échec lors de l'envoi de l'e-mail !";
    ?>
    <script>
      setTimeout(() => {
        location.replace("view-issue-book.php");
...
<head>
  ...
  <title>Système de gestion de bibliothèque || Voir les livres empruntés</title>
...
<body onload="preloader()">
  ...
  <section class="home-section">
    ...
    <div class="control-panel">
      <h4>Tous les livres empruntés</h4>
      <div class="main">
        <?php
        if (isset($_SESSION['lib-name'])) {
        ...
        ?>
          ...
          ?>
            <p class="error">Aucun livre n'a encore été emprunté</p>
          <?php
          }
        } else {
          ?>
